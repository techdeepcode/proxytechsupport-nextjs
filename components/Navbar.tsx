...
export default function Navbar({ variant = 'light' }: Props) {
  ...
  const openDropdownNow = (key: 'jobSupport' | 'locations' | 'interview') => {
    clearCloseDropdownTimer();
    setOpenDropdown(key);
  };

  const closeDropdownSoon = (delayMs = 140) => {
    clearCloseDropdownTimer();
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      const nav = navRef.current;
      if (!nav) return;
      if (!nav.contains(target)) setOpenDropdown(null);
    };
          ...
          {mainNavLinks.map((link) => {
            ...
            if (link.dropdownType === 'jobSupport') {
              return (
                <div
                  key={link.label}
                  style={{ position: 'relative', paddingBottom: '0.15rem' }}
